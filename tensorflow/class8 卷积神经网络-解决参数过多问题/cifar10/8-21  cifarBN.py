#批量归一化  BN算法 大大提高精确度  防止梯度爆炸   最大限度保证每次正向传播输出在同一分布上   p205
...
train = tf.placeholder(tf.float32)                    #添加占位符  传入是否是训练状态
...
h_conv1 = tf.nn.relu(batch_norm_layer((conv2d(x_image, W_conv1) + b_conv1),train))       #卷积完 批量归一池化
...
decaylearning_rate = tf.train.exponential_decay(0.04, global_step,1000, 0.9)    #退化学习率
...
for i in range(2000):
  ...
  train_step.run(feed_dict={x:image_batch, y: label_b,train:1},session=sess)          #注入train：1
