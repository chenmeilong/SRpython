#12-4修改  GAN-cls技术实现生成标签匹配的模拟数据    即生成与标签对应的样本
import numpy as np
...
rand_dim = 38  
n_input  = 784
#判别器    增加标签y输入 全连接后 加入到concatx中  后面操作与  12-1相同
...
global_step = tf.train.get_or_create_global_step()#使用MonitoredTrainingSession，必须有   按时间保存模型
