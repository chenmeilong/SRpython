#LSGAN   12-1修改loss函数  LSGAN模拟生成mnist数据     具有WGAN效果   梯度惩罚方式不同
import numpy as np
...
def discriminator(x, num_classes=10, num_cont=2):
    ...
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        disc = slim.fully_connected(shared_tensor, num_outputs=1, activation_fn=tf.nn.sigmoid)    #用sigmoid函数激活
...
loss_d = tf.reduce_sum(tf.square(disc_real-1) + tf.square(disc_fake))/2     #改为平方差形式
