#ESPCN 对flowers数据 低分辨率 重建高分辨率图像    需要在slim文件夹下运行
...
# 剪辑图片为统一大小 
distorted_image = tf.image.resize_image_with_crop_or_pad(image, height, width)#剪辑尺寸，不够填充 0
################################################
images, labels = tf.train.batch([distorted_image, label], batch_size=batch_size)    # tf.train.batch利用一个tensor的列表或字典来获取一个batch数据
print(images.shape)

x_smalls = tf.image.resize_images(images, (np.int32(height/2), np.int32(width/2)))#  缩小2*2倍
x_smalls2 = x_smalls/255.0
#还原
x_nearests = tf.image.resize_images(x_smalls, (height, width),tf.image.ResizeMethod.NEAREST_NEIGHBOR)   #邻近值插值算法
x_bilins = tf.image.resize_images(x_smalls, (height, width),tf.image.ResizeMethod.BILINEAR)                #双线性插值算法
x_bicubics = tf.image.resize_images(x_smalls, (height, width),tf.image.ResizeMethod.BICUBIC)                #双立方插值算法变化
...
y_predt = tf.depth_to_space(net,2)          #多张合成到1张    2代表尺寸为2*2的块

y_pred = y_predt*255.0
y_pred = tf.maximum(y_pred,0)       #用法tf.maximum(a,b),返回的是a,b之间的最大值，
