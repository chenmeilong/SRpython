...
x_nearest = tf.image.resize_nearest_neighbor(x_small, (28, 28))   #邻近值插值算法
x_bilin = tf.image.resize_bilinear(x_small, (28, 28))             #双线性插值算法


#建立espcn网络结构
net = slim.conv2d(x_small, 64, 5)   #5*5卷积  输出64通道图片
net =slim.conv2d(net, 32, 3)             #3*3卷积  32通道
net = slim.conv2d(net, 4, 3)
net = tf.depth_to_space(net,2)           #多张合成到1张    2代表尺寸为2*2的块
