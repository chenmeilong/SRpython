...
word_num_map = dict(zip(words, range(words_size)))     #样本与向量的对应关系

print('字表大小:', words_size)     
wordlabel = get_ch_lable_v(training_file,word_num_map)  #生成样本向量
...
n_hidden3 = 512
# tf Graph input
x = tf.placeholder("float", [None, n_input,1])      #4个字输入
wordy = tf.placeholder("float", [None, words_size])  #预测第五个字

x1 = tf.reshape(x, [-1, n_input])
x2 = tf.split(x1,n_input,1)
# 2-layer LSTM, each layer has n_hidden units.  每层有 n_hidden个隐藏 单元
rnn_cell = rnn.MultiRNNCell([rnn.LSTMCell(n_hidden1),rnn.LSTMCell(n_hidden2),rnn.LSTMCell(n_hidden3)])

# generate prediction
outputs, states = rnn.static_rnn(rnn_cell, x2, dtype=tf.float32)     #静态rnn
