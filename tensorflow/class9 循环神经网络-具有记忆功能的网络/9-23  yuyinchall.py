...
wav_path='F:/pythoncode/tensorflow/class9 循环神经网络-具有记忆功能的网络/data_thchs30/data_thchs30/train'
label_file='F:/pythoncode/tensorflow/class9 循环神经网络-具有记忆功能的网络/data_thchs30/doc/trans/train.word.txt'
...
def next_batch(labels, start_idx = 0,batch_size=1,wav_files = wav_files):
    ...
    # Verify that the start_idx is not larger than total available sample size #验证start idx
    if start_idx >= filesize:
        start_idx = -1

    # Pad input to max_time_step of this batch  #使用pad方式输入序列
    source, source_lengths = pad_sequences(source)#如果多个文件将长度统一，支持按最大截断或补0
    sparse_labels = sparse_tuple_from(target)#转成稀疏矩阵      #9-24源码

    return start_idx,source, source_lengths, sparse_labels

next_idx,source,source_len,sparse_lab = next_batch(labels,0,batch_size)
print(len(sparse_lab))                                              #sparse_lab 文字转换成 向量 生成的稀疏矩阵
print(np.shape(source))
#print(sparse_lab)
t = sparse_tuple_to_texts_ch(sparse_lab,words)     #稀疏矩阵 向量 转文字
print(t[0])
