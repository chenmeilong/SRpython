#简单循环神经网络，递归减法   没有调用tf库  ，裸写的代码    详见p216
...
## 计算0-256的二进制表示
largest_number = pow(2,binary_dim)    #最大数据
...
synapse_0 = (2*np.random.random((input_dim,hidden_dim)) - 1)*0.05 #维度为2*16， 2是输入维度，16是隐藏层维度   #隐藏层权重
synapse_1 = (2*np.random.random((hidden_dim,output_dim)) - 1)*0.05            #输出层权重
synapse_h = (2*np.random.random((hidden_dim,hidden_dim)) - 1)*0.05            #循环节点权重
# => [-0.05, 0.05)，

# 用于存放反向传播的权重更新值
synapse_0_update = np.zeros_like(synapse_0)    #没有调用tf库  ，裸写的代码  所以有反向更新值
...
for j in range(10000):
    ...
    a = int2binary[a_int] # binary encoding  转二进制
    ...
    # moving along the positions in the binary encoding   正向传播
    for position in range(binary_dim):#循环遍历每一个二进制位
        
        # generate input and output生成输入和输出
        ...
        layer_2_deltas.append((layer_2_error)*sigmoid_output_to_derivative(layer_2)) #把每一个时间点的误差导数都记录下来 #存储每个时间点输出层的误差
        overallError += np.abs(layer_2_error[0])#总误差
    
        d[binary_dim - position - 1] = np.round(layer_2[0][0]) #记录下每一个预测bit位     预测值
        
        # store hidden layer so we can use it in the next timestep  保存隐藏层
        layer_1_values.append(copy.deepcopy(layer_1))#记录下隐藏层的值，在下一个时间点用
    
    future_layer_1_delta = np.zeros(hidden_dim)  #反向传播初始化
    ...
    if(j % 800 == 0):
        ...
        print("True:" + str(c))
        out = 0
        for index,x in enumerate(reversed(d)):   #转成10进制
            out += x*pow(2,index)
