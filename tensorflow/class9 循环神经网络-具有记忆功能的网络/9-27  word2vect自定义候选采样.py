#9-26改  def build_dataset(words, n_words):   改为 手动指定词频率样本生成候选词的方法，代替原来的 nce_loss 中的默认选词方法
import numpy as np
...
def build_dataset(words, n_words):
  ...
  """Process raw inputs into a dataset.""" #建立数据集
  count = [['UNK', -1]]
  count.extend(collections.Counter(words).most_common(n_words - 1))
  
  dictionary = dict()
  vocab_freqs = []      #定义词频数据  list
  for word, nvocab in count:
    dictionary[word] = len(dictionary)
    vocab_freqs.append(nvocab)        #加入字典每一个词频
  data = list()
  unk_count = 0
  for word in words:
...
vocab_freqs[0] = 90   #手动将第一个改为频率最大的
