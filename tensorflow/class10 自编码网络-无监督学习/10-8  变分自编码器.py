...
z_mean = tf.add(tf.matmul(h1, weights['mean_w1']), weights['mean_b1'])           #h1连接两个输出网络
z_log_sigma_sq = tf.add(tf.matmul(h1, weights['log_sigma_w1']), weights['log_sigma_b1'])

 # sample from gaussian distribution 高斯分布样本
eps = tf.random_normal(tf.stack([tf.shape(h1)[0], n_hidden_2]), 0, 1, dtype = tf.float32)
z =tf.add(z_mean, tf.multiply(tf.sqrt(tf.exp(z_log_sigma_sq)), eps))   #公式
...
reconstructionout = tf.matmul(h2out, weights['w3'])+ weights['b3']      #生成指定数据用的，不参与模型

# cost
reconstr_loss = 0.5 * tf.reduce_sum(tf.pow(tf.subtract(reconstruction, x), 2.0))   #生成数据与原始数据的损失
latent_loss = -0.5 * tf.reduce_sum(1 + z_log_sigma_sq
                                   - tf.square(z_mean)
                                   - tf.exp(z_log_sigma_sq), 1)    #比较 生成数据 与高斯分布的距离
...
with tf.Session() as sess:
    ...
    grid_x = norm.ppf(np.linspace(0.05, 0.95, n))    #norm.ppf  按百分比由大到小排列后的标准高斯分布中取值
    grid_y = norm.ppf(np.linspace(0.05, 0.95, n))     #np.linspace(0.05, 0.95, n)  将高斯分布数据 由大到小排列  详见 P383
