#条件变分自编码模型，通过指定标签输入 生成对应类别的mnist 模拟数据
import numpy as np
...
weights = {
    ...
    'wlab1': tf.Variable(tf.truncated_normal([n_labels, n_hidden_1],   #输入标签特征转换
                                   stddev=0.001)),
...
h2=tf.nn.relu( tf.matmul(zall, weights['w2'])+ weights['b2'])  #注意w2 =12
