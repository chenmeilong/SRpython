#mnist数据集加入噪声，在自编码网络中训练  防止过拟合 加入dropout 选择学习     输入为添加噪声的，输出为没有添加噪声的 强迫 学习 去掉噪声
import numpy as np
...
def denoise_auto_encoder(_X, _weights, _biases, _keep_prob):
    ...
    layer_1out = tf.nn.dropout(layer_1, _keep_prob)        #dropout选择学习
    layer_2 = tf.nn.sigmoid(tf.add(tf.matmul(layer_1out, _weights['h2']), _biases['b2'])) 
    layer_2out = tf.nn.dropout(layer_2, _keep_prob)         #dropout选择学习
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        for i in range(num_batch):
            ...
            batch_xs_noisy = batch_xs + 0.3*np.random.randn(batch_size, 784)   #加入0.3倍高斯噪音
            feeds = {x: batch_xs_noisy, y: batch_xs, dropout_keep_prob: 0.5}     #droout 选择学习防止 过拟合
