#自编码模型 进行 分布式模型储存与载入 使一层可以单个环节 训练
import numpy as np
...
tf.reset_default_graph()   #解决P376的坑
...
savedir = "f:/pythoncode/tensorflow/class10 自编码网络-无监督学习/log/"
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        saver.save(sess, savedir + 'stacked_auto_encoder.ckpt', global_step=epoch)   #每次迭代后 都将模型保存
...
with tf.Session() as sess:
    ...
    saver.restore(sess, savedir +"stacked_auto_encoder.ckpt-" + str(load_epoch))   #训练开始时 模型读入  第一个网络不需要读入
