...
class MainWindow(tcp_logic.TcpLogic, udp_logic.UdpLogic, web_logic.WebLogic):
    def __init__(self, num):
        super(MainWindow, self).__init__(num)      #执行父类的初始化函数
        self.client_socket_list = list()      #空list
        self.another = None
        self.link = False
    ...
    def connect(self, ):         #####重写了父类  ui中的connect方法   再父类的初始化方法中调用了一次
        """
        控件信号-槽的设置
        :param : QDialog类创建的对象
        :return: None/
        ...
        super(MainWindow, self).connect()                              # 必须有这行代码去初始化父类的  connect方法

        self.pushButton_link.clicked.connect(self.click_link)
        self.pushButton_unlink.clicked.connect(self.click_unlink)
        self.pushButton_get_ip.clicked.connect(self.click_get_ip)       #获取电脑ip按钮
    ...
    def click_get_ip(self):
        ...
        """
        # 获取本机ip
        self.lineEdit_ip_local.clear()          #清除显示
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)   #TCP和UDP
        try:
