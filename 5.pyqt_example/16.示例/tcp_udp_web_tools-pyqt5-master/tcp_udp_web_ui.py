...
class ToolsUi(QDialog):
    #QDialog对话框  对话框是顶级窗口   一般用Qwidget 这个既可以做顶级窗口又可以嵌入到别的窗口中
    # 信号槽机制：设置一个信号，用于触发接收区写入动作
    signal_write_msg = QtCore.pyqtSignal(str)         #自定义事件   右边的edit显示位置填写内容  的信号
    #？？？？？？？？？不理解为什么用自定义信号   直接插入内容也能实现这个功能
    ...
    def __init__(self, num):
        ...
        self.num = num
        self._translate = QtCore.QCoreApplication.translate        #翻译      传参（类名字，翻译的内容）  返回翻译的结果字符串

        self.setObjectName("TCP-UDP")              #给Qt实例起名字
        self.resize(640, 480)
        ###############################################
        self.setAcceptDrops(False)               #禁止拖放   这行应该可以不用写
        self.setSizeGripEnabled(False)          #是否显示尺寸调整控件    这行可以不用写
        ###############################################

        # 定义控件
        self.pushButton_get_ip = QtWidgets.QPushButton()         #获取ip按钮
        self.pushButton_link = QtWidgets.QPushButton()           #连接网络
        self.pushButton_unlink = QtWidgets.QPushButton()         #断开链接
        self.pushButton_clear = QtWidgets.QPushButton()          #清除消息
        self.pushButton_exit = QtWidgets.QPushButton()           #退出按钮
        self.pushButton_send = QtWidgets.QPushButton()          #发送
        self.pushButton_dir = QtWidgets.QPushButton()         #选择路径
        self.pushButton_else = QtWidgets.QPushButton()        #窗口多开another
        self.label_port = QtWidgets.QLabel()         #端口号
        self.label_ip = QtWidgets.QLabel()          #本机IP
        self.label_rev = QtWidgets.QLabel()        #目标IP
        self.label_send = QtWidgets.QLabel()        #发送区域
        self.label_sendto = QtWidgets.QLabel()    #目标IP
        self.label_dir = QtWidgets.QLabel()         #请选择index.html所在的文件夹
        self.label_written = QtWidgets.QLabel()        #Written by Wangler2333
        self.lineEdit_port = QtWidgets.QLineEdit()
        self.lineEdit_ip_send = QtWidgets.QLineEdit()
        self.lineEdit_ip_local = QtWidgets.QLineEdit()   #本机IP后的输入框
        self.textEdit_send = QtWidgets.QTextEdit()         #数据发送窗口
        self.textBrowser_recv = QtWidgets.QTextBrowser()   #数据接收窗口   只能显示
        self.comboBox_tcp = QtWidgets.QComboBox()          #下拉框
        ...
        self.label_rev.setFont(font)          #应该只是设置发送接收标签的字体格式
    ...
    def ui_translate(self):
        ...
        self.label_rev.setText(self._translate("TCP-UDP", "目标IP"))
    ...
    def write_msg(self, msg):
        ...
        :return: None
        """
        self.textBrowser_recv.insertPlainText(msg)        #数据接收窗口
        # 滚动条移动到结尾
        self.textBrowser_recv.moveCursor(QtGui.QTextCursor.End)  #移动鼠标
