...
token.fit_on_texts(train_text)       #按照单词出现的次数排序  取前2000个单词  #转为词数字   字典
x_train_seq = token.texts_to_sequences(train_text)
x_test_seq = token.texts_to_sequences(test_text)       #变成数字串
x_train = sequence.pad_sequences(x_train_seq, maxlen=100)     #大于100  就截取前面的文字  ，小于100就在前面补0
...
model = Sequential()
model.add(Embedding(output_dim=32,input_dim=2000, input_length=100))       #单词嵌入是使用密集的矢量表示来表示单词和文档的一类方法。
...
pad_input_seq = sequence.pad_sequences(input_seq, maxlen=100)   #100  多删少补
